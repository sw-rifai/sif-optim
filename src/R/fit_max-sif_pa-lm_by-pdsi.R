...
#  Rscript --no-save --no-restore --verbose src/R/fit_max-sif_pa-lm_by-pdsi.R > outputFile.Rout 2>&1
...
# Filter for max SIF ---------------------------------------------------
sdat <- dat[,lst:=round(lst,1)][,.(sif = max(sif,na.rm=T), 
  pdsi = mean(pdsi,na.rm=T)), 
  by=.(lst, id,dc,xc,yc,year)]
snobs <- sdat[,.(nobs = .N),by=.(id,dc)]
sdat <- merge(sdat,snobs,by=c('id','dc'))
...
gc(full=T)
arrow::write_parquet(all_out, sink=paste0("../data_general/proc_sif-optim/mod_fits_max_sif/",
  "lc-",lc_class,"-max-sif-fits_",Sys.Date(),".parquet"),
  compression = 'snappy')
