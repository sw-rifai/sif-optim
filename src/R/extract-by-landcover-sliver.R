...
dseq <- expand_grid(#region = paste0("region_",1:5), 
            year = 2018:2020)
# dseq <- anti_join(dseq,dunzo,by=c("region","year")) %>%
...
for(idx in 1:dim(dseq)[1]){
 ...
 roi_name <- 'sliver' #dseq[idx,'region']$region
 ...
 # roi <- eval(as.name(roi_name))
  ...
  # if(all(st_get_dimension_values(p,1) == st_get_dimension_values(m,1))==F){
    n_bands <- dim(p)[3]
    p <- st_warp(p,m[,,,1:n_bands],use_gdal = F)
    p <- st_as_stars(p)
  # }
  all(st_get_dimension_values(p,1) %in% st_get_dimension_values(m,1))
  st_get_dimension_values(m,1) %in% unique(dat$x) %>% table
  # p <- st_crop(p,s_bb,epsilon = 0.999)
  # p_bb <- st_bbox(p)
  # gc(full=T)
  d_p <- p %>% as.data.table()
  gc(full=T)
  ...
  # st_get_dimension_values(p,1) %in% unique(d_p$x)
  # st_get_dimension_values(m,1) %in% unique(dat$x) %>% table
  # st_get_dimension_values(p,1) %in% st_get_dimension_values(m,1)
  # st_get_dimension_values(p,2) %in% st_get_dimension_values(m,2)
  # unique(d_p$x) %in% unique(dat$x) %>% table
  # unique(d_p$y) %in% unique(dat$y) %>% table
  # unique(d_p$x) %>% sort %>% .[1:10] %>% diff
  # unique(dat$x) %>% sort %>% .[1:10] %>% diff
  # unique(d_p$x) %>% sort %>% diff
  # unique(dat$x) %>% sort %>% diff
  dat <- d_p[dat,roll=T]
  

  (fname_out <- paste0("lc-",lc_class,"_","sliver_","sif_","lst_","1km_",i_year,"_.parquet"))
